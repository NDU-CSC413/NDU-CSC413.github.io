...
$$
\begin{aligned}
\mathbf{for }  h=0 & \mathbf{ to } \log n -1\mathbf { do}\\
 &\mathbf{for } & k=0 \text{ in multiples of } 2^{h+1} \mathbf{do}\\
&S[k+2^{h+1}-1]=S[k+2^h-1]+S[k+2^{h+1}-1]
\end{aligned}
$$

We can write multiples of $2^{h+1}$ as $\alpha\cdot 2^{h+1}$ and rewrite the loops as

$$
s=1\\
\begin{aligned}
\mathbf{for }  d=\frac{n}{2} & \mathbf{ to } 1 \text{ divided by 2 } \mathbf { do}\\
 &\mathbf{for } & \alpha =0 \mathbf{ to } d-1 \mathbf{do}\\
&S[(2\alpha+2)\cdot s-1]+=S[(2\alpha+1)\cdot s -1 ]
\end{aligned}
$$
